...
from models.vad_architecture_v13 import TriModalVAD, DynamicNormalPrototypes
from utils.video_utils import load_config, set_seed
from utils.dataset_v13 import TriModalVADDataset, get_dataloaders_v13
from utils.losses import VADLoss, SelfTrainingLoss
from utils.metrics import compute_auroc, interpolate_scores
from utils.frame_eval_v13 import compute_frame_level_auroc_v13


def build_class_balanced_sampler(
    dataset: TriModalVADDataset,
...
def train(config_path: str) -> None:
    ...
    # --- DataLoaders ---
    train_loader, test_loader = get_dataloaders_v13(config)
    ...
    # --- Model & Prototypes ---
    model = TriModalVAD.from_config(config).to(device)

    # V14: Optional warm-start from a pretrained checkpoint (e.g. V12)
    pretrained_path = config["training"].get("pretrained_checkpoint", None)
    if pretrained_path:
        ...
        print(f"  [WARM-START] Loaded {len(loaded)} params, skipped {len(skipped)} (new/reshaped)")
        if skipped:
            print(f"  [WARM-START] Skipped keys: {skipped[:10]}{'...' if len(skipped) > 10 else ''}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (visual, text, flow, i3d, labels) in enumerate(pbar):
            visual = visual.to(device)   # (B, 32, D)
            text   = text.to(device)     # (B, 32, D)
            flow   = flow.to(device)     # (B, 32)
            i3d    = i3d.to(device)      # (B, 32, 2048)
            labels = labels.to(device)   # (B,)

            scores, norms, guided = model(visual, text, flow, i3d_features=i3d)   # (B, 32), (B, 32), (B, 32, D)
        ...
        if len(test_loader.dataset) > 0:
            ...
            if should_eval_fl:
                ...
                frame_auroc = compute_frame_level_auroc_v13(
...
def evaluate_epoch(
    ...
    for visual, text, flow, i3d, labels in test_loader:
        visual = visual.to(device)
        text   = text.to(device)
        flow   = flow.to(device)
        i3d    = i3d.to(device)

        scores, _, _ = model(visual, text, flow, i3d_features=i3d)  # (B, 32)
