...
def compute_frame_level_auroc_v13(
    ...
    for idx in range(len(dataset)):  # type: ignore[arg-type]
        item = dataset[idx]          # (visual, text, flow, i3d, label)
        visual_item = item[0]
        text_item   = item[1]
        flow_item   = item[2]
        i3d_item    = item[3]
        ...
        i3d_t    = i3d_item.unsqueeze(0).to(device)

        with torch.no_grad():
            scores, _, _ = model(visual_t, text_t, flow_t, i3d_features=i3d_t)
